...
- Compatibility alias: auth endpoints are available under both `/api/v1/auth/*` and `/api/v1/user/*` for older clients.
- Frontend API base URL defaults to `http://localhost:3005` when opened from another localhost port (e.g. `4173`).

- Non-JSON API failures are parsed gracefully in frontend auth/CRUD flows so you see real status/error details instead of a generic network error.
